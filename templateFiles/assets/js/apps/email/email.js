    ...
    .controller('EmailListCtrl', ['$scope', 'emails', '$stateParams', '$state', '$timeout', '$window',
        function($scope, emails, $stateParams, $state, $timeout, $window) {
            ...
            emails.all().then(function(list) {
                ...
                $scope.emails = list;
                console.log("hahahah wlwi ",list)
            });

            $scope.$on('ws:new_email' ,function(_, new_email){


                    console.log("in websocket by email")
                    var group = new_email.date.slice(4, 10)
                    var emails = $scope.emails
                    var bool = false;
                    if(emails) {
                        for (var i = 0; i < emails.length; i++) {
                            if (emails[i].group == group) {
                                emails[i].list.unshift(new_email)
                                bool = true
                                continue;
                            }
                        }
                        if (bool == false) {
                            var abc = {group: group, list: []}
                            abc.list.push(new_email)
                            emails.unshift(abc)
                        }
                        $scope.emails = emails
                    }
